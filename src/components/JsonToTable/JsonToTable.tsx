...
import { JSONObjectKeys, JSONObjectType,capitalizeLabel } from "./JsonToTableUtils";
    ...
    private renderValues = (values: string[]) => {
        return (
            <tr key={`__j2t_trArrString`}>
                {values.map(k => {
                    return this.renderCell({content: capitalizeLabel(k)});
                })}
            </tr>
        );
    };
    ...
    private renderRowHeader = (label: string) => {
        return (
            <div key={`__j2t_rw${label}`}>
                <strong> {capitalizeLabel(label)}</strong>
            </div>
        );
    };
